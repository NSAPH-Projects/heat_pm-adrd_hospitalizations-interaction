...
data = data %>%
  select(-c(pm25, no2, ozone, max_temp, min_humid, heat_index) ) %>%
...
colnames(data)[105:118] = paste0("orig_heat_index_lag_", 1:14)

data = data %>%
  rename(orig_heat_index_lag_0 = heat_index_lag_0 ) %>%
  cbind(lag_dat_perc)

colnames(data)[119:133] = paste0("heat_index_lag_", 0:14)
